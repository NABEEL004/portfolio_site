  ...
  return (
    <div className="flex justify-center items-center p-1 sm:p-3 md:p-6 bg-[#D4A269] absolute top-0 left-0 h-full w-full">
        <motion.div 
            ...
            className='h-1 w-1 bg-[#FDFAE3] opacity-0 rounded-full fixed z-[999] left-[230px] sm:left-[250px] bottom-[220px]'>
        </motion.div>
        <div className='flex flex-col mx-10 justify-between items-center sm:items-start w-[800px]'>
            ...
            <motion.div className="h-[40px]">
                {/* <Link to='/about'> */}
                    <button onClick={() => setClicked(true)} className={`${display ? 'opacity-100' : 'opacity-0'} font-title font-black bg-[#FDFAE3] rounded-full p-5 m-5 px-10 text-[#D4A269] text-lg sm:text-xl md:text-3xl hover:scale-105 active:scale-95 transition duration-200`}>Don't Click This!</button>
                {/* </Link> */}
            </motion.div>
        </div>
